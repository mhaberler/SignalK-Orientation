...
#if defined(M5CORE2) || defined(M5STICKC)
#include <M5Unified.h>
#endif
...
#define BOARD_ACCEL_MAG_I2C_ADDR                                               \
  (0x1F)                           ///< I2C address on Adafruit breakout board
...
#define PIN_SWITCH_CAL_SAVE                                                    \
  (0) // When at SWITCH_ACTIVE_STATE, saves mag calibration
...
#define PIN_SWITCH_CAL_SAVE                                                    \
  (36) // When brought LOW, will save magnetic calibration
#define SWITCH_ACTIVE_STATE (0) // Input is LOW when Switch is pushed
#endif
#endif
...
#if defined(M5STICKC)
#define PIN_I2C_SDA (32) // Adjust to your board. A value of -1
#define PIN_I2C_SCL (33) // will use default Arduino pins.
#define PIN_SWITCH_CAL_SAVE                                                    \
  (37) // BTN_A - When brought LOW, will save magnetic calibration
#define SWITCH_ACTIVE_STATE (0) // Input is LOW when Switch is pushed
#endif
...
void setup() {
   ...
   * parameters have predefined paths in the Signal K spec, so may be changed to
   * suit.
   *
  ...
  // const char* kSKPathMagInclination  =
  // "orientation.calibration.maginclination"; const char* kSKPathMagBValue =
  // "orientation.calibration.magmagnitude"; const char* kSKPathMagBValueTrial =
   ...
   *
       sensors->attitude
              ...
              ->heading
                       ->deviation (adjusts compass deviation with Curve
   Interpolator)
  ...
  // For other parameters, uncomment and modify the appropriate path(s) from the
  // following or create new paths as needed.
  ...
  /*
   * Create the desired outputs from the orientation sensor. Note that the
   * physical sensor is read at whatever rate is specified in the Sensor Fusion
   * library's build.h file (#define FUSION_HZ), currently set to 40 Hz. Fusion
   ...
   * via a parameter's Value Settings->Report Interval entry in the web
   * interface. It is not necessary that all the values be output at the same
   * rate (for example, it likely makes sense to report temperature at a slower
   * rate).
   */
  ...
  sensor_heading
      ...
      // AngleCorrection normalizes to [0..2Pi] range, when CurveInterpolator
      // output < 0 or > 2*Pi
      ->connect_to(new AngleCorrection(0.0, 0.0, ""))
      ->connect_to(
  ...
  sensor_cal_fit->connect_to(new SKOutputFloat(kSKPathMagFit, ""));
  ...
  sensor_cal_candidate->connect_to(new SKOutputFloat(kSKPathMagFitTrial, ""));
  ...
  sensor_cal_order->connect_to(new SKOutputFloat(kSKPathMagSolver, ""));
  ...
  auto *sensor_mag_noise = new OrientationValues(
      ...
      ORIENTATION_REPORTING_INTERVAL_MS * 10, "");
  sensor_mag_noise->connect_to(new SKOutputFloat(kSKPathMagNoise, ""));
  ...
  //  const uint kReadInterval = 100;
  //  String testpath = "somepath";
  //  auto* button_watcher = new DigitalInputChange(
  //      PIN_SWITCH_CAL_SAVE, INPUT_PULLUP, kReadInterval, testpath);
  auto *button_watcher =
      new DigitalInputChange(PIN_SWITCH_CAL_SAVE, INPUT_PULLUP, CHANGE, "");
  ...
  auto *sensor_temperature =
      new OrientationValues(orientation_sensor, OrientationValues::kTemperature,
                            3001, kConfigPathTemperature);
  sensor_temperature
      ...
      ->connect_to(new SKOutputFloat(
          kSKPathTemperature, kConfigPathTemperature_SK, metadata_temperature));
   ...
   * @see https://github.com/BjarneBitscrambler/SignalK-Orientation/wiki
   *
   * If the sensor is mounted differently, or you prefer an alternate
   * nomenclature, the Get___() methods in sensor_fusion_class.cpp can be
   * adjusted.
   */
